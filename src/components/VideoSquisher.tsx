  ...
  return (
    <section className="border border-[#ff7448]/35 bg-[#1d1512] p-5 shadow-[0_16px_40px_-28px_rgba(0,0,0,0.9)]">
            ...
            {ENCODER_PRESETS.map((preset) => {
              ...
              return (
                <button
                  ...
                  title={!supported ? "This engine is not supported on your device" : undefined}
                  onClick={(e) => {
                    if (!supported) {
                      e.preventDefault();
                      return;
      ...
      <button
        ...
        title={isCompressing ? "Compression in progress..." : isInspecting ? "Inspecting video..." : undefined}
        onClick={(e) => {
