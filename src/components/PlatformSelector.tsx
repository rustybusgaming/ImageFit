    ...
    <div className="border border-white/10 bg-[#151714] p-5 shadow-[0_16px_40px_-28px_rgba(0,0,0,0.9)]">
      ...
      <form onSubmit={addCustomPreset} className="mt-5 flex items-end gap-3 border border-white/10 bg-[#1b1e1a] p-3">
        ...
        <button
          ...
          title={!customW || !customH ? "Enter both width and height to add a custom preset" : undefined}
          onClick={(e) => {
            if (!customW || !customH) {
              e.preventDefault();
            }
          }}
          className="inline-flex h-[38px] items-center gap-2 border border-[#d7ff47]/45 bg-[#20251a] px-4 text-sm font-semibold text-[#d7ff47] transition hover:border-[#d7ff47] hover:bg-[#292f21] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#d7ff47] focus-visible:ring-offset-2 focus-visible:ring-offset-[#1b1e1a] aria-disabled:opacity-50 aria-disabled:cursor-not-allowed"
        >
